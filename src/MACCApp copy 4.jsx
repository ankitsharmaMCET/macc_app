/* FULL MACCApp.jsx — Hooks-order fixed, Add-measure working, hoverable MACC columns, DB-aware multi-line drivers, adoption & interpolation, carbon price, NPV/IRR, scrollable wizard, PNG export, timeseries viewer */
  ...
  return lines.map((line)=>{
    ...
    if (obj.details) {
      try {
        ...
        // Leave as string or handle error
      }
    }
    return obj;
  });
...
function MeasureWizard({ onClose, onSave, sectors, currency, carbonPrice }) {
  const YEARS = [2020, 2025, 2030, 2035, 2040, 2045, 2050];
  const BASE_YEAR = YEARS[0];
  const CR_TO_RUPEES = 10_000_000;

  const [tab, setTab] = useState("template"); // default to template view
  ...
  const [adoption, setAdoption] = useState(YEARS.map((y, i) => i === 0 ? 0 : 0.2 * i)); // Better default adoption
  ...
  const [fuelLines, setFuelLines] = useState([
    { id:1, name: FUELS[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() }
  ]);
  const [rawLines, setRawLines] = useState([
    { id:1, name: RAW[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() }
  ]);
  const [transLines, setTransLines] = useState([
    { id:1, name: TRANSPORT[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() }
  ]);
  const [wasteLines, setWasteLines] = useState([
    { id:1, name: WASTE[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() }
  ]);
  const [elecLines, setElecLines] = useState([
    { id:1, state: ELECTRICITY[0]?.state || "India", priceOv:null, priceEscPctYr:0, efEscPctYr:0, efOvPerYear: makeEmptyEf(), deltaMWh: makeZeros() }
  ]);
  ...
  const computed = useMemo(() => {
    const perYear = YEARS.map((year, i) => {
      ...
      // Cash flow in ₹
      // Note: Cash flows are defined as incoming (positive) and outgoing (negative).
      // Here, all costs are outgoing, so they should be negative values.
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-3 rounded-xl border">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-xs px-2 py-1 rounded border" onClick={() => addLine(fuelLines, setFuelLines, { name: FUELS[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() })}>+ Add fuel line</button>
                  </div>
                  {fuelLines.map((ln) => {
                    const base = FUELS.find(x => x.name === ln.name);
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <label className="text-sm col-span-2">Fuel
                            <select className="mt-1 border rounded-xl px-3 py-2 w-full" value={ln.name} onChange={e=>updateLine(fuelLines, setFuelLines, ln.id, { name: e.target.value })}>
                              {FUELS.map(x=> <option key={x.name}>{x.name}</option>)}
                            </select>
                          </label>
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-xs px-2 py-1 rounded border" onClick={() => addLine(rawLines, setRawLines, { name: RAW[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() })}>+ Add raw line</button>
                  </div>
                  {rawLines.map((ln) => {
                    const base = RAW.find(x => x.name === ln.name);
                    const unit = base?.unit || "-";
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <label className="text-sm col-span-2">Raw material
                            <select className="mt-1 border rounded-xl px-3 py-2 w-full" value={ln.name} onChange={e=>updateLine(rawLines, setRawLines, ln.id, { name: e.target.value })}>
                              {RAW.map(x=> <option key={x.name}>{x.name}</option>)}
                            </select>
                          </label>
                          <label className="text-sm">Price override (₹/{unit})
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-xs px-2 py-1 rounded border" onClick={() => addLine(transLines, setTransLines, { name: TRANSPORT[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() })}>+ Add transport line</button>
                  </div>
                  {transLines.map((ln) => {
                    const base = TRANSPORT.find(x => x.name === ln.name);
                    const unit = base?.unit || "-";
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <label className="text-sm col-span-2">Transport
                            <select className="mt-1 border rounded-xl px-3 py-2 w-full" value={ln.name} onChange={e=>updateLine(transLines, setTransLines, ln.id, { name: e.target.value })}>
                              {TRANSPORT.map(x=> <option key={x.name}>{x.name}</option>)}
                            </select>
                          </label>
                          <label className="text-sm">Price override (₹/{unit})
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-xs px-2 py-1 rounded border" onClick={() => addLine(wasteLines, setWasteLines, { name: WASTE[0]?.name || "", priceOv:null, efOv:null, priceEscPctYr:0, efEscPctYr:0, delta: makeZeros() })}>+ Add water/waste line</button>
                  </div>
                  {wasteLines.map((ln) => {
                    const base = WASTE.find(x => x.name === ln.name);
                    const unit = base?.unit || "-";
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <label className="text-sm col-span-2">Water/Waste
                            <select className="mt-1 border rounded-xl px-3 py-2 w-full" value={ln.name} onChange={e=>updateLine(wasteLines, setWasteLines, ln.id, { name: e.target.value })}>
                              {WASTE.map(x=> <option key={x.name}>{x.name}</option>)}
                            </select>
                          </label>
                          <label className="text-sm">Price override (₹/{unit})
                ...
                <div className="md:col-span-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-xs px-2 py-1 rounded border" onClick={() => addLine(elecLines, setElecLines, { state: ELECTRICITY[0]?.state || "India", priceOv:null, priceEscPctYr:0, efEscPctYr:0, efOvPerYear: makeEmptyEf(), deltaMWh: makeZeros() })}>+ Add electricity line</button>
                  </div>
                  {elecLines.map((ln) => {
                    const base = ELECTRICITY.find(x => x.state === ln.state) || ELECTRICITY[0];
                    return (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 mt-2">
                          <label className="text-sm sm:col-span-2">State
                            <select className="mt-1 border rounded-xl px-3 py-2 w-full" value={ln.state} onChange={e=>updateLine(elecLines,setElecLines,ln.id,{ state: e.target.value })}>
                              {ELECTRICITY.map(e=> <option key={e.state}>{e.state}</option>)}
                            </select>
                          </label>
...
function MACCAppInner() {
  ...
  const [carbonPrice, setCarbonPrice] = useLocalStorage("macc_carbon_price", 0); // ₹/tCO2
  ...
  // Costs shown in MACC are adjusted by carbon price: effectiveCost = saved cost − carbon price
  ...
  const quad = useMemo(() => {
    if (maccData.length < 3) return null;
  ...
  // Budget-to-target (effective costs)
  ...
  // Wizard (conditionally mounted to avoid hook-order issues)
  ...
  const importCSV = (rows) => {
    const parsed = rows.map((r,i)=>({
      ...
      details: r.details, // Preserve details object
    }));
  ...
  // Hover tooltip state for MACC rectangles
  const [hoverInfo, setHoverInfo] = useState(null); // { seg, x, y }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex-1 relative" ref={maccRef}>
              <ResponsiveContainer width="100%" height={360}>
                {costModel === 'step' ? (
                  <ComposedChart data={axisData} margin={{ top:10, right:10, left:10, bottom:40 }}>
                    ...
                    {/* Interactive overlay to capture hover on the full rectangle area */}
                    <Customized
                      component={(props) => {
                        ...
                        const offL = (offset?.left || 0);
                        const offT = (offset?.top  || 0);
              ...
              {hoverInfo && hoverInfo.seg && (
                ...
                >
                  ...
                  <div>Marginal cost: {currency} {formatNumber(hoverInfo.seg.cost)} /tCO₂</div>
                </div>
              )}
        ...
        <section className="bg-white rounded-2xl shadow border p-6">
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-gray-100 text-gray-700">
                  ...
                  <th className="p-2 text-right">Saved cost ({currency}/t)</th>
          ...
          <div className="mt-3 text-xs text-gray-500">
            CSV columns: <code>name, sector, abatement_tco2, cost_per_tco2, selected</code>.
          </div>
        </section>
